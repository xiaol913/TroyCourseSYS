...
//先根据是否为空值，留下有值的数据
    ...
    //先从GET里提取day的信息，然后根据信息从数据库里提出courseID，把where后的条件加在condition里
    $day=$_GET['day'];
    ...
    if(count($day)>1){
        ...
        for($j=0;$j<(count($day)-1);$j++){
            $dayKey=key($day);
            $dayCondition .= $dayKey.' and ';
            next($day);
        }
        $lastDayKey=key($day);
        $value=$lastDayKey;
        reset($day);
        $dayCondition=$dayCondition.$lastDayKey;
        ...
        if(count($res)>=1){
            for($k=0;$k<(count($res)-1);$k++){
                $result=key($res);
                $condition.=" courseId={$res[$result][$value]}";
                next($res);
                if (count($notNull) >= 1) {
                    for ($i = 0; $i < count($notNull); $i++) {
                        $key = key($notNull);
                        if ($key == 'courseStartTime') {
                            $condition .= ' and '.$key . '>' . $notNull[$key];
                            next($notNull);
                ...
                $condition.=' or';//第一组结束，以OR结尾
            }
            $lastResult=key($res);
            reset($res);//重置指针
            $condition=$condition." courseId={$res[$lastResult][$value]}";
            if (count($notNull) >= 1) {
                for ($i = 0; $i < count($notNull); $i++) {
                    $key = key($notNull);
                    if ($key == 'courseStartTime') {
        ...
        if(count($res)>=1){
            for($k=0;$k<(count($res)-1);$k++){
                $result=key($res);
                $condition.=" courseId={$res[$result][$oneKey]}";
                next($res);
                if (count($notNull) >= 1) {
                    for ($i = 0; $i < count($notNull); $i++) {
                        $key = key($notNull);
            ...
            $lastResult=key($res);
            reset($res);//重置指针
            $condition=$condition." courseId={$res[$lastResult][$oneKey]}";
            if (count($notNull) >= 1) {
                for ($i = 0; $i < count($notNull); $i++) {
                    $key = key($notNull);
                    if ($key == 'courseStartTime') {
